...
async function scrapeEventPage(browserPage, url) {
...
async function collectRenderedCalendarLinks(page) {
    const urls = new Set();
    for (const frame of page.frames()) {
        const links = await frame.locator("a[href]").evaluateAll((els) => els.map((a) => a.href).filter(Boolean)).catch(() => []);
        for (const href of links)
            if (isHopEventUrl(href))
                urls.add(href);
    }
    return [...urls];
}
async function extractCalendarCards(page) {
    const found = [];
    ...
    for (const frame of page.frames()) {
        ...
        for (const selector of selectors) {
            ...
            for (let i = 0; i < Math.min(n, 300); i++) {
                ...
                if (!text || text.length < 8 || text.length > 1500)
                    continue;
                const dt = parseDateTime(text);
                if (!dt)
                    continue;
...
async function clickCalendarEventCards(page) {
    const found = [];
...
async function diagnosticFrames(page) {
    const rows = [];
    for (const frame of page.frames()) {
...
export async function crawl() {
    ...
    try {
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            for (const event of await extractCalendarCards(page))
                all.set(event.uid, event);
            for (const event of await clickCalendarEventCards(page))
                all.set(event.uid, event);
