...
function copenhagenOffset(date) {
...
function parseWeekEvents(text) {
    ...
    for (let d = 0; d < days.length; d++) {
        ...
        const exact = possible.find(dt => dt.toLocaleDateString("en-US", { weekday: "long" }).toUpperCase() === match[1].toUpperCase());
        if (exact)
...
async function logControls(page) {
    for (const [i, frame] of page.frames().entries()) {
        const controls = await frame.locator('button, [role="button"], a').evaluateAll((els) => els.slice(0, 200).map(el => ({
            tag: el.tagName,
